...
-- Part 1 of the Expired Listing outreach build.
--
-- 1. Per-lead subject-property fields (the expired listing's address/city) so
--    the sequence merge fields {{property_address}} / {{city}} have a source.
--    (leads had no address columns; skip-traced lists carry them per row.)
-- 2. The 'expired_listing' sequence: 4 EMAILS, delays Day 0 / 3.5 / 7 / 13,
--    trigger_type='manual' (enrolled by the agent, single or batch).
--    Steps carry mode:"literal" — the copy is Sara's verbatim, sent as written
--    with only merge fields filled (NOT AI-drafted like the other sequences),
--    plus preview_text. The cron honours mode:"literal" (see sequences-cron.js).
--
-- Idempotent.
